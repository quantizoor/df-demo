...
import { MountedVolumeCanonicalEvaluatorReplayLedger } from "../../src/cloud/mounted-volume-evaluator-replay-ledger.js";
import type {
...
function claim(requestId = "request-001", requestHash = "a".repeat(64)) {
  return {
    requestId,
    requestHash,
...
describe("mounted-volume evaluator replay ledger", () => {
  it("keeps a failed transport claim burned across controller handoff", async () => {
    const root = await mkdtemp(join(tmpdir(), "df-evaluator-replay-test-"));
    temporaryDirectories.push(root);
    const first = new MountedVolumeCanonicalEvaluatorReplayLedger(
      options(root, "1".repeat(64), "a".repeat(48)),
    );

    await expect(first.claim(claim())).resolves.toBe(true);
    await first.close();

    const successor = new MountedVolumeCanonicalEvaluatorReplayLedger(
      options(root, "2".repeat(64), "b".repeat(48)),
    );
    await expect(successor.claim(claim())).resolves.toBe(false);
    await expect(successor.claim(claim("request-002"))).resolves.toBe(false);
    await expect(successor.claim(claim("request-003", "b".repeat(64)))).resolves.toBe(true);
    await successor.close();
  });

  it("linearizes concurrent claims so exactly one caller burns the panel", async () => {
    const root = await mkdtemp(join(tmpdir(), "df-evaluator-replay-race-test-"));
    temporaryDirectories.push(root);
    const ledger = new MountedVolumeCanonicalEvaluatorReplayLedger(
  ...
  it("rejects malformed claims before durable state changes", async () => {
    const root = await mkdtemp(join(tmpdir(), "df-evaluator-replay-invalid-test-"));
    temporaryDirectories.push(root);
    const ledger = new MountedVolumeCanonicalEvaluatorReplayLedger(
      options(root, "4".repeat(64), "d".repeat(48)),
    );
    ...
    await expect(ledger.claim(claim("constructor"))).rejects.toThrow();
