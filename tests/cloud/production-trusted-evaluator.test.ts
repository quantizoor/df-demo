...
import type { BehavioralReleaseArtifact } from "../../src/evaluator/behavioral-release-producer.js";
import type {
...
describe("production trusted evaluator artifact overlay", () => {
  it("delegates only cache attestations to the governed fallback", async () => {
    ...
    await expect(overlay.source.locate(query("cache-attestation"))).resolves.toEqual(expected);
  ...
  it("never falls through when a behavioral hash is missing or orphaned", async () => {
    ...
    await expect(overlay.source.locate(query("behavioral-evidence"))).resolves.toBeUndefined();
  ...
  it("binds committed behavioral canonical bytes and rechecks them on read", async () => {
    let stored: BehavioralReleaseArtifact | undefined = behavioralArtifact();
    ...
    const reference = await overlay.source.locate(query("behavioral-evidence"));
    ...
    await expect(overlay.reader.readUtf8(reference, 1_024)).resolves.toBe(raw);

    stored = undefined;
    await expect(overlay.reader.readUtf8(reference, 1_024)).rejects.toBeInstanceOf(
      ProductionMountedVolumeTrustedEvaluatorError,
    );
  });
  ...
  it("reserves behavioral URIs against fallback namespace confusion", async () => {
    ...
    await expect(overlay.source.locate(query("cache-attestation"))).rejects.toBeInstanceOf(
      ProductionMountedVolumeTrustedEvaluatorError,
    );
  });
  ...
  it("rejects a store response detached from the requested purpose", async () => {
    ...
    await expect(overlay.source.locate(query("behavioral-evidence"))).rejects.toBeInstanceOf(
      ProductionMountedVolumeTrustedEvaluatorError,
    );
  });
});
