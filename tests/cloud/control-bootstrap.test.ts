...
function completeEnvironment(): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DF_PI_PACKAGE_LOCK_SHA256: "472f0726dc79f3b38df58d8a8bce96bf56fbf993a134b49aabc54947b8461e59",
    ...
    DF_CONTROL_NETWORK_ALLOW_DOMAINS: "app.daytona.io,api.github.com,github.com",
    DF_CONTROL_CPU: "4",
    DF_CONTROL_MEMORY_GIB: "8",
    DF_CONTROL_DISK_GIB: "20",
    [PRODUCTION_OPTIMIZE_BOOTSTRAP_DESCRIPTOR_ENVIRONMENT_NAME]: canonicalJson(
      optimizeBootstrapDescriptor(),
    ),
  };
}

function freeStageEnvironment(stage: "offline" | "probe"): NodeJS.ProcessEnv {
  const imageDigest = `sha256:${"a".repeat(64)}`;
  return {
    GITHUB_ACTIONS: "true",
    RUNNER_ENVIRONMENT: "github-hosted",
    ...
    ...(stage === "probe"
      ? {
          ...
          DF_EVALUATOR_IMAGE_DIGEST: imageDigest,
          DF_CONTROL_DAYTONA_SECRET_SOURCE: "DF_CONTROL_DAYTONA",
          DF_CONTROL_NETWORK_ALLOW_DOMAINS: "app.daytona.io",
        }
      : {}),
    DF_DAYTONA_VOLUME_ID: "dark-factory-state",
    DF_DAYTONA_VOLUME_SUBPATH: "campaigns/campaign-one",
...
class Sandbox implements DaytonaControlSandbox {
  ...
  readonly volumes: NonNullable<DaytonaControlSandbox["volumes"]>;
  ...
  delete(): Promise<void> {
    this.deleted = true;
    return this.failDelete ? Promise.reject(new Error("provider detail")) : Promise.resolve();
  }
}
...
class Factory implements DaytonaControlClientFactory {
  ...
  create(): Promise<DaytonaControlClient> {
    return Promise.resolve({
      create: (parameters: DaytonaControlCreateParameters): Promise<DaytonaControlSandbox> => {
...
describe("trusted cloud control bootstrap", () => {
  ...
  it("parses a provider probe without optimizer, model, Git, benchmark, budget, or signing inputs", () => {
    ...
    expect(request.controllerDaytonaSecretSource).toBe("DF_CONTROL_DAYTONA");
    expect(request.additionalControllerSecrets).toEqual([]);
    expect("optimizer" in request.configuration).toBe(false);
  });
  ...
  it("requires an exact image-bound authorization before a paid optimize launch", () => {
    expect(() =>
      parseCloudControlBootstrapEnvironment(completeEnvironment(), "optimize", "campaign-one"),
    ...
    expect(
      parseCloudControlBootstrapEnvironment(environment, "optimize", "campaign-one").command,
    ).toBe("optimize");
  });
  ...
  it.each([
    ...
    "LC_ALL",
    PRODUCTION_OPTIMIZE_BOOTSTRAP_DESCRIPTOR_ENVIRONMENT_NAME,
  ])("rejects an organization-secret mapping that can override %s", (targetEnvironmentName) => {
    const environment = completeEnvironment();
    environment["DF_CONTROL_SECRET_BINDINGS_JSON"] = JSON.stringify([
      {
    ...
    expect(() =>
      parseCloudControlBootstrapEnvironment(environment, "probe", "campaign-one"),
    ).toThrow(/unsafe or duplicate target/u);
  });

  it("mounts the campaign subpath, isolates synthetic, and confirms teardown", async () => {
    const environment = completeEnvironment();
    const request = parseCloudControlBootstrapEnvironment(environment, "synthetic", "campaign-one");
    ...
    const receipt = await launchDaytonaControlPlane(
      ...
      factory,
      environment,
      () => new Date(call++ === 0 ? "2026-07-26T10:00:00.000Z" : "2026-07-26T10:05:00.000Z"),
    );
    ...
    expect(factory.parameters?.envVars["DF_TRUSTED_VOLUME_ROOT"]).toBe("/trusted/dark-factory");
    expect(
      factory.parameters?.envVars[PRODUCTION_OPTIMIZE_BOOTSTRAP_DESCRIPTOR_ENVIRONMENT_NAME],
    ).toBeUndefined();
    expect(factory.parameters?.envVars["DF_OPTIMIZER_MODEL"]).toBeUndefined();
    expect(factory.parameters?.envVars["DF_EVALUATED_MODEL"]).toBeUndefined();
    expect(factory.parameters?.envVars["DF_GITHUB_SECRET_SOURCE"]).toBeUndefined();
    expect(factory.parameters?.envVars["DF_SIGNING_KEY_ID"]).toBeUndefined();
    expect(JSON.stringify(factory.parameters)).not.toContain("github-bootstrap-only-secret");
  ...
  it("grants a provider probe only the nested Daytona organization secret", async () => {
    const environment = completeEnvironment();
    const request = parseCloudControlBootstrapEnvironment(environment, "probe", "campaign-one");
    const factory = new Factory();
    let call = 0;
    ...
    expect(factory.parameters?.domainAllowList).toBe("api.github.com,app.daytona.io,github.com");
  });
  ...
  it("forwards only the validated canonical descriptor into an optimize sandbox", async () => {
    ...
    environment["DF_PAID_RUN_AUTHORIZATION"] =
      `RUN:campaign-one:${environment["DF_CONTROL_IMAGE_DIGEST"]}`;
    const request = parseCloudControlBootstrapEnvironment(environment, "optimize", "campaign-one");
    const factory = new Factory();
    let call = 0;
  ...
  it("rejects a missing or cross-campaign optimize descriptor", () => {
    ...
    missing["DF_PAID_RUN_AUTHORIZATION"] = `RUN:campaign-one:${missing["DF_CONTROL_IMAGE_DIGEST"]}`;
    delete missing[PRODUCTION_OPTIMIZE_BOOTSTRAP_DESCRIPTOR_ENVIRONMENT_NAME];
    expect(() =>
      parseCloudControlBootstrapEnvironment(missing, "optimize", "campaign-one"),
    ...
    detached[PRODUCTION_OPTIMIZE_BOOTSTRAP_DESCRIPTOR_ENVIRONMENT_NAME] = canonicalJson(
      optimizeBootstrapDescriptor("another-campaign"),
    );
    expect(() =>
      parseCloudControlBootstrapEnvironment(detached, "optimize", "campaign-one"),
    ).toThrow(/descriptor/u);
  });
  ...
  it("invalidates a successful command if provider teardown is not confirmed", async () => {
    ...
    await expect(launch).rejects.toBeInstanceOf(CloudControlBootstrapError);
