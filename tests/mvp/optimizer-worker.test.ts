...
describe("MVP optimizer worker delivery", () => {
  it("pins the cloud entrypoint, Claude version, model effort, and tool boundary", () => {
    ...
    expect(MVP_OPTIMIZER_ALLOWED_TOOLS).toEqual([
      "Read",
      "Edit",
      "Write",
      "Grep",
  ...
  it("builds an explicit mounted-file invocation", () => {
    ...
    expect(invocation).toMatchObject({
      ...
      inputPath:
        "/workspace/df-state/inbox/optimizer-input.json",
      outputPath:
        "/workspace/df-state/outbox/candidate-proposal.json",
      environment: {
        CI: "true",
...
describe("MVP optimizer protocol boundary", () => {
  it("accepts only the strict task-free optimizer input", () => {
    expect(() =>
      assertTaskFreeMvpOptimizerInput(optimizerInput),
    ).not.toThrow();
    expect(() =>
      assertTaskFreeMvpOptimizerInput({
        ...optimizerInput,
  ...
  it("accepts a bounded generic Pi harness proposal and rejects protected content", () => {
    ...
    expect(() =>
      assertMvpCandidateChangedFiles([
        "packages/coding-agent/test/hidden-grader.test.ts",
      ]),
    ...
    expect(() =>
      assertMvpCandidateProposal({
        ...
        hypothesisSummary:
          "The Terminal-Bench grader rewards a special response.",
        interventionSummary:
          "Route that task name to a stored answer.",
  ...
  it("uses scoped HTTPS header auth and never a shell or credential URL", async () => {
    ...
    expect(source).toContain(
      "Fail-closed MVP optimizer filesystem boundary.",
    );
    expect(source).toContain(
