...
try {
  ...
  const request = parseCloudControlBootstrapEnvironment(process.env, command, campaignId);
  ...
  process.stderr.write(
    `${JSON.stringify({
      ...
      message: "Trusted cloud bootstrap failed closed. Inspect protected cloud logs.",
    })}\n`,
  );
  process.exitCode = 1;
}
