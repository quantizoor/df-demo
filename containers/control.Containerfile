...
# No fallback tag is permitted. The publishing workflow accepts only an
# operator-reviewed name@sha256:digest value.
ARG ROLE_BASE_IMAGE
...
RUN test -x /usr/local/bin/node \
    ...
    && pnpm build \
    && test -f /build/dist/cloud/control-plane.js \
    && pnpm prune --prod
...
LABEL org.opencontainers.image.source="https://github.com/quantizoor/df-demo" \
      ...
      io.parallaxai.dark-factory.role="control" \
